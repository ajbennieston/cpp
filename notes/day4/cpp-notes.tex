...
\verb|boost::lexical_cast<>()| is a template function which converts from the argument type to the template parameter type. See section \ref{sec:templates} for more on templates. The nice thing about \verb|boost::lexical_cast<>()| is that it makes the intention clear---you state that you want to convert from a string to an integer---even though it (often) uses string streams behind the scenes!
...
\subsection{Template functions}\label{sec:templates_functions}
...
This is the power of \emph{template functions}. By writing code such as the following, we can provide the compiler with a template, into which it substitutes the type required at compiletime:
...
In the same way as for template functions, it is possible to create \emph{template classes}. The standard library includes many of these, but we'll look at a simple example; converting the \verb|SaferDynamicArray| class from the day three slides into a template class.
...
Note that you can also use template functions within template classes, providing for incredibly powerful, incredibly generic templates!
